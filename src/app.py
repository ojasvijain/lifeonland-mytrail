...
df_trails = pd.read_csv('data/50_trails.csv', encoding='utf-8')
df_uploads = pd.read_csv('data/uploads.csv')
...
@app.callback(
    ...
    [State('geo', 'position'),  # Position state from geolocation
     ...
     State('upload-image', 'filename')],  # Position error state from geolocation
    prevent_initial_call=True
)
def handle_upload(contents, local_date, position, position_error, filename):
    if contents is None:
    ...
    new_upload = {
        ...
        'upload_img_path': f'data/uploads/{len(df_uploads)+1}_{filename[0]}',
        'upload_species': species
    }
    df_uploads.loc[len(df_uploads)] = new_upload
    
    with open(f'data/uploads/{len(df_uploads)}_{filename[0]}', 'wb') as f:
        f.write(decoded)
        
    df_uploads.to_csv('data/uploads.csv', index=False)
...
def display_image_marker(contents, trail, status, zoom):
    ...
    if not df_uploads.empty:
        ...
        for _, row in filtered_df.iterrows():
            image_url = b64_image(row['upload_img_path'])
